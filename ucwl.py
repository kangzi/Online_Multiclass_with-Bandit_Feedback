...
class UCWL:
    def __init__(self, x, y, test_x=None, test_y=None, eta = 0.5, k = 100.0, test_interval:int=100):
        """
        ...
        self.Sigma = np.ones((self.K, self.n))  # Matrix A (only diagonal element)
        self.xi = 1 + self.phi**2
        self.psy = 1 + self.phi**2/2
    ...
    def train(self, t):
        ...
        for count, i in enumerate(seq):
            ...
            proposed_label = self._det_proposed_label(x, wx, self.k)
    ...
    def _det_alpha_beta(self, x, proposed_label, z_t):
        v_t = (self.Sigma[proposed_label]*self._fun(x)).dot(self._fun(x))
        m_t = z_t*self.w[proposed_label].dot(self._fun(x))
